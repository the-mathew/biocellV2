<template>
  <div class="home-wrapper" :class="{ 'zoom-in-effect': isZooming }">
...
function confirmStart() {
  ...
  // Esperar a que termine la animación de zoom antes de navegar
  setTimeout(() => {
    navigateTo({ path: '/dashboard', query: { user: nameInput.value.trim() } })
  }, 1300)
}
...
.zoom-in-effect {
  animation: superZoom 1.4s cubic-bezier(0.7, 0, 0.2, 1) forwards;
  pointer-events: none;
}

@keyframes superZoom {
  0% { transform: scale(1); filter: brightness(1) blur(0); }
  30% { filter: brightness(1.2) blur(1px); }
  100% { transform: scale(25); filter: brightness(2) blur(10px); opacity: 0; }
}
